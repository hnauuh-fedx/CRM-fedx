...
## Repository-Aware Coding Prompts

For Template G, Template H, or Template M used to create or edit code in this repository:

- Read `AGENTS.md` before composing the prompt. Read `CODEX.md` as well only when that file exists.
- Add a `Project Constraints` section containing only rules relevant to the requested change.
- Scope files to `apps/web/`, `apps/api/`, `packages/shared/`, `database/`, `uploads/`, or `docs/` as applicable.
- Require the target coding agent to report modified files and verification performed.

Example section to include in a generated coding prompt:

```txt
Project Constraints:
- Read and follow AGENTS.md before editing; also follow CODEX.md if it exists.
- Keep changes within [relevant directory/module].
- Preserve the established React/Vite/TypeScript or Node.js/TypeScript/PostgreSQL stack.
- Use existing shared types/constants and validate protected API input where applicable.
```
