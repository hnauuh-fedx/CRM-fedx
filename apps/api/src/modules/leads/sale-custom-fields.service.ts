...
export type RuntimeCustomFieldEntityType =
  | SaleCustomFieldEntityType
  | "MARKETING_CAMPAIGN"
  | "MARKETING_FORM"
  | "ADMISSION_PROFILE"
  | "ADMISSION_DOCUMENT"
...
type CustomFieldValueEntityType = RuntimeCustomFieldEntityType | "LEAD";
...
function getMaxFiles(field: Field) {
  ...
  return typeof rules.maxFiles === "number" &&
    imageFileLimitOptions.has(rules.maxFiles)
    ? rules.maxFiles
    : 1;
}
...
function readValue(field: Field, row: any): Normalized {
  if (!row) return null;
  if (
    ["TEXT", "TEXTAREA", "EMAIL", "PHONE", "SELECT", "PROVINCE"].includes(
      field.field_type,
    )
  )
...
function normalize(field: Field, raw: unknown): Normalized | null {
  ...
  if (
    ...
    return raw.trim();
  if (
    field.field_type === "PROVINCE" &&
    typeof raw === "string" &&
    vietnamProvinceCodes.has(raw)
  )
    return raw;
  if (field.field_type === "FILE") {
    ...
    if (
      files.length === 0 ||
      files.length > getMaxFiles(field) ||
      files.some(
  ...
  if (
    ...
    Array.isArray(raw) &&
    raw.every((value) => typeof value === "string" && options.has(value)) &&
...
async function saleEntityContext(
  user: AuthUser,
  entityType: SaleCustomFieldEntityType,
  entityId: string,
  institutionProgramId?: string,
) {
  if (entityType === "SALE_ACTIVITY") {
    ...
    return activity
      ? {
          id: activity.id,
          programId: activity.leads?.institution_program_id ?? null,
        }
      : null;
  }
  ...
  return reminder
    ? {
        id: reminder.id,
        programId: reminder.leads?.institution_program_id ?? null,
      }
    : null;
}

function serializeField(
  field: Field & { custom_field_groups: any },
  value: Normalized,
  user: AuthUser,
  canEdit: boolean,
) {
  ...
  return {
    ...
    defaultValue:
      field.is_sensitive && !viewSensitive ? undefined : field.default_value,
...
export async function getSaleCustomFieldDefinitions(
  user: AuthUser,
  entityType: SaleCustomFieldEntityType,
  ...
  const lead = leadId
    ? await visibleLead(user, leadId, institutionProgramId)
    : null;
  if (leadId && !lead) return null;
  const programId =
    lead?.institution_program_id ?? institutionProgramId ?? null;
  ...
  return {
    fields: fields.map((field) =>
      serializeField(field, readValue(field, null), user, canEdit),
    ),
  };
}

export async function getSaleCustomFields(
  user: AuthUser,
  entityType: SaleCustomFieldEntityType,
  entityId: string,
  canEdit: boolean,
  ...
  const context = await saleEntityContext(
    user,
    entityType,
    entityId,
    institutionProgramId,
  ...
  const fields = await prisma.custom_fields.findMany({
    ...
    include: {
      custom_field_groups: true,
      custom_field_values: {
        where: { entity_type: entityType, entity_id: context.id },
      },
    },
    orderBy: [
      { custom_field_groups: { display_order: "asc" } },
  ...
  return {
    fields: fields.map((field) =>
      serializeField(
        field,
        readValue(field, field.custom_field_values[0]),
        user,
        canEdit,
      ),
    ),
...
export async function saveSaleCustomFieldValues(
  tx: Tx,
  user: AuthUser,
  entityType: CustomFieldValueEntityType,
  entityId: string,
  programId: string | null,
  values: SaleCustomFieldInput[],
  ip?: string,
) {
  if (values.length === 0) return { ok: true as const };
  const fields = await tx.custom_fields.findMany({
    where: {
      ...applicable(entityType, programId),
      id: { in: values.map((value) => value.fieldId) },
    },
  });
  ...
  const prepared = values.map((value) => ({
    field: map.get(value.fieldId)!,
    value: normalize(map.get(value.fieldId)!, value.value),
    raw: value.value,
  }));
  if (
    prepared.some(
  ...
  if (
    prepared.some((item) => item.value === null && item.field.is_required) ||
    prepared.some(
      (item) => item.value === null && item.raw !== null && item.raw !== "",
    )
  )
  ...
  for (const item of prepared) {
    const old = await tx.custom_field_values.findUnique({
      where: valueWhere(item.field.id, entityType, entityId),
    });
...
async function runtimeEntityContext(
  user: AuthUser,
  entityType: RuntimeCustomFieldEntityType,
  entityId: string,
  institutionProgramId?: string,
) {
  if (entityType === "SALE_ACTIVITY" || entityType === "SALE_REMINDER")
    return saleEntityContext(user, entityType, entityId, institutionProgramId);
  if (entityType === "MARKETING_CAMPAIGN") {
    const row = await prisma.campaigns.findFirst({
      where: {
        ...
        ...(institutionProgramId
          ? { institution_program_id: institutionProgramId }
          : {}),
        ...(!user.permissions.includes("campaign.view_all") &&
        !user.permissions.includes("campaign.update")
          ? { created_by: user.id }
          : {}),
      },
      select: { id: true, institution_program_id: true },
    ...
    return row
      ? { id: row.id, programId: row.institution_program_id ?? null }
      : null;
  }
  if (entityType === "MARKETING_FORM") {
    const row = await prisma.marketing_forms.findFirst({
      where: {
        id: entityId,
        ...(!user.permissions.includes("campaign.view_all") &&
        !user.permissions.includes("marketing_form.manage")
          ? { created_by: user.id }
          : {}),
        ...(institutionProgramId
          ? {
              OR: [
                {
                  campaigns: {
                    is: { institution_program_id: institutionProgramId },
                  },
                },
                { campaign_id: null },
              ],
      ...
      select: {
        id: true,
        campaigns: { select: { institution_program_id: true } },
      },
    });
    return row
      ? {
          id: row.id,
          programId:
            row.campaigns?.institution_program_id ??
            institutionProgramId ??
            null,
        }
      : null;
  ...
  if (entityType === "ADMISSION_PROFILE") {
    const row = await prisma.admission_profiles.findFirst({
      where: {
        id: entityId,
        ...(institutionProgramId
          ? { institution_program_id: institutionProgramId }
  ...
  if (entityType === "ADMISSION_DOCUMENT") {
    const row = await prisma.admission_documents.findFirst({
      where: {
        id: entityId,
        ...(institutionProgramId
          ? { leads: { is: { institution_program_id: institutionProgramId } } }
          : {}),
      },
      select: { id: true, leads: { select: { institution_program_id: true } } },
    });
    return row
      ? { id: row.id, programId: row.leads?.institution_program_id ?? null }
      : null;
  }
  if (entityType === "ADMISSION_STATUS") {
    const row = await prisma.admission_statuses.findUnique({
      where: { id: entityId },
      select: { id: true },
    });
    return row ? { id: row.id, programId: institutionProgramId ?? null } : null;
  }
  const row = await prisma.majors.findFirst({
    where: {
      id: entityId,
      ...(institutionProgramId
        ? { institution_program_id: institutionProgramId }
...
export async function getRuntimeCustomFieldDefinitions(
  user: AuthUser,
  entityType: RuntimeCustomFieldEntityType,
  canEdit: boolean,
  institutionProgramId?: string,
  ...
  const fields = await prisma.custom_fields.findMany({
    where: applicable(entityType, institutionProgramId ?? null),
    include: { custom_field_groups: true },
    orderBy: [
      { custom_field_groups: { display_order: "asc" } },
      { display_order: "asc" },
...
export async function getRuntimeCustomFields(
  user: AuthUser,
  entityType: RuntimeCustomFieldEntityType,
  entityId: string,
  canEdit: boolean,
  ...
  const context = await runtimeEntityContext(
    user,
    entityType,
    entityId,
    institutionProgramId,
...
export async function saveRuntimeCustomFields(
  user: AuthUser,
  entityType: RuntimeCustomFieldEntityType,
  entityId: string,
  values: SaleCustomFieldInput[],
