# Prompt Failure Patterns Reference

38 patterns that cause poor prompts or repeated corrections. Read this file when the user asks to fix a weak prompt or when diagnosing why a prompt is underperforming.
...
| 38 | **Missing repository instructions** | Coding prompt ignores existing project rules | Read `AGENTS.md` and, when present, `CODEX.md`; then include relevant stack, path, validation, and verification requirements in the prompt. |
