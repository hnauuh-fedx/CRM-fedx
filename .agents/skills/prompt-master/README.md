...
A Codex skill that creates or improves prompts for AI tools while carrying applicable repository constraints into coding-agent task briefs.
...
This repository skill is stored at `.agents/skills/prompt-master/`. Codex discovers it from `SKILL.md` frontmatter and loads the Markdown body only when a request matches the skill description.

The required entry file is `SKILL.md`. Supporting instructions live in `references/templates.md` and `references/patterns.md`, which Codex reads only when relevant to the requested prompt.
...
## Codex Repository Usage

In this repository, `prompt-master` is used when the requested output is a prompt for a coding agent, not when Codex implements a change directly.

- For direct code changes, Codex follows `AGENTS.md` and any additional repository instruction file that exists.
- For a prompt asking Codex, Claude Code, Cursor, or another agent to edit this project, the skill reads `AGENTS.md` first and reads `CODEX.md` only if present, then includes relevant project constraints in the generated prompt.
- Keep coding-agent prompts scoped to the affected module and require changed-file and verification summaries.

Example:

```txt
Write a Codex prompt to add student profile validation in apps/api while following this repository's instructions.
```
