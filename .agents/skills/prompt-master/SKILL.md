---
name: prompt-master
description: Creates or improves prompts for AI tools, including Codex, Claude Code, Cursor, Windsurf, and other coding agents. Use when the user asks to write, fix, improve, or adapt a prompt or task brief for another AI tool. For prompts that generate or edit code in this repository, read AGENTS.md and any available CODEX.md, then carry applicable constraints into the prompt. Do not use for direct code implementation requests.
---
...
This skill creates prompts; it is not the coding policy for direct implementation work.

When the generated prompt will ask Codex or another coding agent to create, edit, refactor, debug, or review code in this repository:

1. Read `../../../AGENTS.md` before writing the prompt. If `../../../CODEX.md` exists, read it as an additional repository instruction source.
2. Extract only constraints relevant to the requested task, including file scope, stack, module placement, API/auth rules, validation, and verification.
3. Include those constraints in the generated prompt as explicit requirements, using repository-relative paths.
4. Include completion criteria requiring a summary of changed files and performed checks.
5. Do not invent project requirements that are not present in the user request or project instruction files.

When the user asks Codex to implement code directly rather than write a prompt, do not invoke this skill; follow the repository instructions directly.
